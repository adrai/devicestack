...
Connection.prototype.executeCommand = function(command, callback) {
  try {
    ...
    function run() {
      self.commandQueue.push({ command: command, callback: callback });
      ...
      if (!self.isSleeping) {
        process.nextTick(function() {
          try {
            ...
            if (this.log) this.log('Error while sending command: ' + command.constructor.name + '!');
    ...
    if (this.log) this.log('Error while executing command: ' + command.constructor.name + '!');
    if (callback) { callback(new Error('Error while executing command: ' + command.constructor.name + '!')); }
  }
};
  ...
  try {
    ...
    function run() {
      if (ignoreQueue) {
        ...
        self.taskQueue.push({ task: task, callback: callback });
    ...
    if (task.initialize) {
      try {
        ...
        if (this.log) this.log(err.name + ' while initializing task: ' + command.constructor.name + '!');
        if (callback) { callback(err); }
        return;
      }
    }
    ...
    if (this.log) this.log('Error while executing task: ' + task.constructor.name + '!');
    if (callback) { callback('Error while executing task: ' + task.constructor.name + '!'); }
  }
