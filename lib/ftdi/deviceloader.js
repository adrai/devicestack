  ...
  ftdi.find(this.vendorId, this.productId, function(err, ports) {
    ...
    var devices = _.map(resPorts, function(p) {
      var found = _.find(self.oldDevices, function(dev) {
        return dev.get('deviceSettings').locationId === p.deviceSettings.locationId &&
               dev.get('deviceSettings').vendorId === p.deviceSettings.vendorId &&
               dev.get('deviceSettings').productId === p.deviceSettings.productId &&
               dev.get('deviceSettings').serial === p.deviceSettings.serial;
      });
      if (found) {
        return found;
      } else {
